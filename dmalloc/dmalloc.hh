...
extern struct dmalloc_stats global_stats;
...
class dbg_allocator {
   public:
    ...
    template <typename U>
    dbg_allocator(dbg_allocator<U>&) noexcept {}

    T* allocate(size_t n) {
        return reinterpret_cast<T*>(dmalloc(n * sizeof(T), "?", 0));
    }
    void deallocate(T* ptr, size_t) { dfree(ptr, "?", 0); }
};
template <typename T, typename U>
inline constexpr bool operator==(const dbg_allocator<T>&,
                                 const dbg_allocator<U>&) {
    return true;
}
template <typename T, typename U>
inline constexpr bool operator!=(const dbg_allocator<T>&,
